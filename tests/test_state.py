...
def test_apply_fire_damage_adds_burning_effect(humanoid_fighter: FighterState):
    ...
    assert not any(eff.name == C.EFFECT_BURNING and eff.metadata.get(C.TARGETED_PART) == part_name for eff in fighter.debuffs)
    fighter.apply_damage_to_part(part_name, 15, C.DAMAGE_TYPE_FIRE)
    
    burning_effects = [eff for eff in fighter.debuffs if eff.name == C.EFFECT_BURNING and eff.metadata.get(C.TARGETED_PART) == part_name]
...
def test_apply_piercing_damage_adds_bleeding_effect(humanoid_fighter: FighterState):
    ...
    assert not any(eff.name == C.EFFECT_BLEEDING and eff.metadata.get(C.TARGETED_PART) == part_name for eff in fighter.debuffs)
    fighter.apply_damage_to_part(part_name, 10, C.DAMAGE_TYPE_PIERCING)
    
    bleeding_effects = [eff for eff in fighter.debuffs if eff.name == C.EFFECT_BLEEDING and eff.metadata.get(C.TARGETED_PART) == part_name]
...
def test_apply_delta_applies_wounds(humanoid_fighter: FighterState):
    ...
    delta = {
        C.WOUNDS: [
            {C.TARGETED_PART: part_name, C.VALUE: damage_val, C.TYPE: C.DAMAGE_TYPE_PIERCING}
        ]
    }
    fighter.apply_delta(delta)
    final_hp_sum = sum(layer.max_hp for layer in fighter.parts[part_name].layers)
...
def test_apply_delta_vital_part_destruction_in_wound(humanoid_fighter: FighterState):
    ...
    delta = {
        C.WOUNDS: [
            {C.TARGETED_PART: part_name, C.VALUE: heart_hp + 5, C.TYPE: C.DAMAGE_TYPE_PIERCING}
        ]
    }
    fighter.apply_delta(delta)
    assert fighter.parts[part_name].status == C.IS_DESTROYED
...
def test_apply_effects_multiple_effects_simultaneously(humanoid_fighter: FighterState):
    ...
    burning_effect = Effect(name=C.EFFECT_BURNING, magnitude=1.0, ttl=1, on_apply="Burning", metadata={C.TARGETED_PART: torso})
    bleeding_effect = Effect(name=C.EFFECT_BLEEDING, magnitude=2.0, ttl=1, on_apply="Bleeding", metadata={C.TARGETED_PART: leg})
